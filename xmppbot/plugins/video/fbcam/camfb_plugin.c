...
#define FBDEV_DEVNAME_STR "/dev/fb0"

typedef struct fbcam_camera
{
  x_object xobj;
...
static void
__gobee_yuv_set_data(fbcam_camera_t *thiz,
    gobee_img_frame_t yuv, void *data);
static int
__xqvfb_write_frame_to_encoder_hlp(
    fbcam_camera_t *thiz , gobee_img_frame_t yuv);

static int
__xqvfb_check_writehandler_hlp(fbcam_camera_t *thiz)
{
    x_object *tmp;
...
{
  fbcam_camera_t *thiz = (fbcam_camera_t *)(void *)obj;
  thiz->cam_qwartz.data = (void *) obj;

  ENTER;

  thiz->vfd = x_open(FBDEV_DEVNAME_STR,O_RDWR);
  ...
  TRACE("Opened " FBDEV_DEVNAME_STR "... fd=%d\n",thiz->vfd);
...
static void
x_fbcam_cam_on_write_frame(x_object *xobj,
    void *framedata, size_t siz, x_obj_attr_t *attrs)
{
  ...
  fbcam_camera_t *thiz = (fbcam_camera_t *)(void *)xobj;
...
static int
x_fbcam_cam_on_append(x_object *so, x_object *parent)
{
  int ofd;
  fbcam_camera_t *mctl = (fbcam_camera_t *) so;
  ENTER;
...
static x_object *
x_fbcam_cam_on_assign(x_object *this__, x_obj_attr_t *attrs)
{
  fbcam_camera_t *mctl = (fbcam_camera_t *) this__;
  ENTER;
...
static void
x_fbcam_cam_exit(x_object *this__)
{
  fbcam_camera_t *mctl = (fbcam_camera_t *) this__;
  ENTER;
  TRACE("\n");
...
static struct xobjectclass fbcam_cam_class;

static void
___fbcam_cam_on_x_path_event(void *_obj)
{
...
static struct x_path_listener fbcam_cam_bus_listener;
...
{
  ENTER;
  fbcam_cam_class.cname = _XS("camera");
  fbcam_cam_class.psize = (sizeof(fbcam_camera_t) - sizeof(x_object));
  fbcam_cam_class.on_assign = &x_fbcam_cam_on_assign;
  fbcam_cam_class.on_append = &x_fbcam_cam_on_append;
  fbcam_cam_class.commit = &x_fbcam_cam_exit;
  fbcam_cam_class.try_write = &x_fbcam_cam_on_write_frame;

  x_class_register_ns(fbcam_cam_class.cname, &fbcam_cam_class,
      _XS("gobee:media"));

  fbcam_cam_bus_listener.on_x_path_event = &___fbcam_cam_on_x_path_event;
   ...
   * activated.
   */
  x_path_listener_add("/", &fbcam_cam_bus_listener);
  EXIT;
  return;
}

PLUGIN_INIT(x_fbcam_init)
;
