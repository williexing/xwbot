 ...
 *      when n is the digest capacity in bits.  Therefore, this
 ...
 *      a multiple of the capacity of an 8-bit character.
 *
 */
...
SHA1Result(SHA1Context *context, uint8_t Message_Digest[SHA1HashSize])
{
  ...
  for (i = 0; i < SHA1HashSize; ++i)
    {
      Message_Digest[i] = context->Intermediate_Hash[i >> 2] >> 8 * (3 - (i
          & 0x03));
    }
...
/**
 * \return message digest in HEX format which should be freed
 */
char *
sha1_get_sha1(const char *buf, unsigned int len)
{
  SHA1Context sha;
  int i, err;
  ...
  printf("Creating digest for '%s' (%d bytes long)\n", buf, len);
  ...
  err = SHA1Input(&sha, (const uint8_t *) buf, len);
  if (err)
    {
  ...
  if (err)
  ...
  else
    {
      ...
      for (i = 0; i < 20; ++i, _res += 2)
        {
          printf("%02x", Message_Digest[i]);
          sprintf(_res, "%02x", Message_Digest[i]);
        }
    }
