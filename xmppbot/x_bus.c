...
/* forward decls */
static void __x_bus_hotstart(struct x_bus *bus);
...
#ifndef WIN32
#ifdef LOG_XMPPP
    sprintf(lognamebuf,"xmpp_trace_out-%d.xml",rand());
    bus->xmppoutfd = open(lognamebuf, O_CREAT | O_TRUNC | O_SYNC | O_RDWR,
                          S_IRWXU | S_IRWXG);
    sprintf(lognamebuf,"xmpp_trace_in-%d.xml",rand());
    bus->xmppinfd = open(lognamebuf, O_CREAT | O_TRUNC | O_SYNC | O_RDWR,
                         S_IRWXU | S_IRWXG);
#endif
#endif
...
__x_bus_jid_commit(x_object *bus)
{
    ...
    *ptr2++ = '\0';
    ...
    if (!_AGET((struct xobject *) bus, "hostname"))
    {
        ...
        else if (EQ(ptr1, "qip.ru"))
        {
            _ASET((struct xobject *) bus, "hostname", QIP_HOSTNAME);
        }
        else
        {
 ...
 */
static void
__x_bus_on_signal_data(xevent_dom_t *loop, struct ev_io *data, int mask)
{
    int err, i;
    char buf[256];
    struct x_bus *bus;
...
static void
__x_bus_fill_nsmap(struct x_bus *bus, x_obj_attr_t *attrs, const char *ns_to_replace)
{
    int ind;
    char *ptr;
    x_obj_attr_t *p;
    ...
    for (p = attrs->next; p; p = p->next)
    {
        if (!x_strncasecmp(p->key, "xmlns:", sizeof("xmlns:") - 1))
        {
            ...
            if(ns_to_replace && EQ(ptr,ns_to_replace))
            {
                ptr2 = p->val;
            }
        }
    ...
    if (ns_to_replace && ptr2)
    {
        setattr("xmlns", ptr2, attrs);
        x_snprintf(ptr3, sizeof(ptr3) - 1, "xmlns:%s", ns_to_replace);
        setattr(ptr3, NULL, attrs);
    }
}
...
__x_bus_objopen(const char *_name, x_obj_attr_t *attrs, void *dat)
{
    ...
    __x_bus_fill_nsmap(bus, attrs, has_ns ? ns.cbuf : NULL);
 ...
 */
EXPORT_SYMBOL void
x_bus_reset_stream(struct x_bus *bus)
{
    x_object *streamo;
    ...
    streamo = _GNEW("stream", "http://etherx.jabber.org/streams");
...
static void
__x_bus_on_pause(x_object *thiz_)
{
    ENTER;
 ...
 */
static void
__x_bus_hotstart(struct x_bus *bus)
{
    int err;
    ENTER;

    TRACE("connecting xbus...\n");
    ...
    if (ev_is_active(&bus->io_watcher))
    {
        ev_io_stop(bus->obj.loop, &bus->io_watcher);
    }
 ...
 */
static void
__x_bus_finalize(x_object *thiz)
{
    struct x_bus *bus = (struct x_bus *) thiz;
    ENTER;

    TRACE("\n");

    //  signal(SIGPIPE, SIG_IGN);
    TRACE("finalizing xbus...\n");
    /* first commit changes */
    __x_bus_jid_commit(thiz);
    _SETNM(X_OBJECT(bus), _AGET(X_OBJECT(bus), "hostname"));

    TRACE("starting xbus...\n");
    /* start the bus */
    __x_bus_hotstart(bus);

    TRACE("starting EV loop xbus...\n");
#if 0
    ev_run(bus->loop, EVLOOP_NONBLOCK);
#else
...
void
__x_bus_remove_n_stop(x_object *o)
{
    struct x_bus *bus = (struct x_bus *) o;
    ENTER;

#if 0
    ev_break(bus->loop, EVUNLOOP_ALL);
#else
    if(bus->obj.loop)
        ev_unloop(bus->obj.loop, EVUNLOOP_ALL);
#endif
...
xbus_lib_init(void)
{
    ...
    xbus_class.commit = __x_bus_finalize;
    xbus_class.on_remove = __x_bus_remove_n_stop;
