...
static void
__jingle_session_on_initiate(x_object *o)
{
    int err;
    x_object *chan;
    x_object *descr;
    ...
    if (from)
    {
        if (msgo->regs.state == JINGLE_STATE_GNEW)
        {
            sid = _AGET(o, "sid");
   ...
   */
    ...
    msgo->regs.cr0 = JINGLE_CR0_PENDING;
...
static void
__jingle_session_on_terminate(x_object *o)
{
    x_obj_attr_t hints =
    { 0, 0, 0 };
    x_object *_sess_;
    ...
    /* just call api */
    x_session_close(_ENV(o,"from"),o,&hints,0);
#if 0
    if (_sess_)
    {
...
jingle_msg_recv(x_object *to, const x_object *from, const x_object *msg)
{
    ...
    else if (EQ(_XS("session-destroy"),subj))
    {
        ...
        jnglo->iosession = NULL;
        _USBSCRB(from,to);
//        _REFPUT(to,NULL);
//        __jingle_session_send_terminate(to);
    }
    TRACE("\n");
    EXIT;
    return 0;
