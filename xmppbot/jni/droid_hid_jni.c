...
#define HID_VENDOR_ID "GobeeHid_4Android"
...
typedef struct _droid_hid
{
  x_object xobj;
} droid_hid;

static int
droid_hid_on_append(x_object *thiz)
{
  ENTER;
  TRACE("new HIDDEV sid=%s\n",_ENV(thiz,"sid"));
    _ASET(thiz,"Descr","Android HID event generator");
...
static struct xobjectclass droid_hid_clazz;
...
droid_hid_init(void)
{
  droid_hid_clazz.cname = _XS("hidmux");
  droid_hid_clazz.on_append = &droid_hid_on_append;

  x_class_register_ns(droid_hid_clazz.cname,
      &droid_hid_clazz,_XS("gobee:media"));
